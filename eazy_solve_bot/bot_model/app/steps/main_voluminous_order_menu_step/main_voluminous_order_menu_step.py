...
class MainVoluminousOrderStep(StepModel):
    ...
    def update_created_order_status(tg_id: int, order_status_name: str) -> Any:
        created_order_id = MainVoluminousOrderStep.get_created_order_id(tg_id)
        if created_order_id is None:
            return None
    ...
    async def logic(self, data: Any) -> State:
        ...
        if callback_data == ButtonCallbackData.NEW_NOW_ORDER_IMPORTANCE_CALLBACK_DATA:
            ...
            order_id = MainVoluminousOrderStep.get_created_order_id(data.from_user.id)
            ...
            MainVoluminousOrderStep.update_created_order_status(data.from_user.id, STATE_ORDER[OrderStateEnum.ORDER_FROZEN])
            return BotStates.NEW_ORDER_BY_BOT_STEP
        return self.state
