...
class NewOrderStep(StepModel):
    ...
    async def logic(self, data: Any) -> State:
        ...
        if callback_data == ButtonCallbackData.NEW_ORDER_BY_ADMIN_CALLBACK_DATA:
            return BotStates.NEW_ORDER_BY_ADMIN_STEP

        elif callback_data == ButtonCallbackData.NEW_ORDER_BY_BOT_CALLBACK_DATA:
            return BotStates.NEW_ORDER_BY_BOT_STEP
        
        elif callback_data == ButtonCallbackData.UNFINISHED_ORDER_CALLBACK_DATA:
            return BotStates.UNFINISHED_ORDER_STEP
    ...
    def view(self) -> None:
        @self.dp.callback_query_handler(text = ButtonCallbackData.NEW_ORDER_BY_ADMIN_CALLBACK_DATA, state = self.state)
        async def new_order_by_admin_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
        ...
        @self.dp.callback_query_handler(text = ButtonCallbackData.NEW_ORDER_BY_BOT_CALLBACK_DATA, state = self.state)
        async def new_order_by_bot_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
            next_state = await self.logic(callback_query)
            output_data = await self.db(next_state, callback_query)
            msg, keyboard = output_data
        ...
        @self.dp.callback_query_handler(text = ButtonCallbackData.UNFINISHED_ORDER_CALLBACK_DATA, state = self.state)
        async def unfinished_order_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
            next_state = await self.logic(callback_query)
            output_data = await self.db(next_state, callback_query)
            msg, keyboard = output_data
