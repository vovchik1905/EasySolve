...
class AdminMainMenuStep(StepModel):
    ...
    async def logic(self, data: Any) -> State:
        ...
        elif callback_data == ButtonCallbackData.ADMIN_CONTROL_PANEL_CALLBACK_DATA:
            return BotStates.ADMIN_CONTROL_MENU
        return self.state
    ...
    def view(self) -> None:
        ...
        @self.dp.callback_query_handler(text = ButtonCallbackData.ADMIN_CONTROL_PANEL_CALLBACK_DATA, state = self.state)
        async def admin_control_panel_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
            next_state = await self.logic(callback_query)
            output_data = await self.db(next_state, callback_query)
            msg, keyboard = output_data
