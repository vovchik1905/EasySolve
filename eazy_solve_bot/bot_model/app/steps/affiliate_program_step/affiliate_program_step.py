...
class AffiliateProgramStep(StepModel):
    ...
    async def logic(self, data: Any) -> State:
        ...
        if callback_data == ButtonCallbackData.LIST_AFFILIATE_LINKS_CALLBACK_DATA:
            return BotStates.LIST_AFFILIATE_LINKS_STEP

        elif callback_data == ButtonCallbackData.CREATE_AFFILIATE_LINK_CALLBACK_DATA:
            return BotStates.CREATE_AFFILIATE_LINK_STEP
        
        elif callback_data == ButtonCallbackData.AFFILIATE_PROGRAM_STATISTICS_CALLBACK_DATA:
            return BotStates.AFFILIATE_PROGRAM_STATISTICS_STEP
    ...
    def view(self) -> None:
        @self.dp.callback_query_handler(text = ButtonCallbackData.LIST_AFFILIATE_LINKS_CALLBACK_DATA, state = self.state)
        async def list_affiliate_links_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
        ...
        @self.dp.callback_query_handler(text = ButtonCallbackData.CREATE_AFFILIATE_LINK_CALLBACK_DATA, state = self.state)
        async def create_affiliate_link_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
            next_state = await self.logic(callback_query)
            output_data = await self.db(next_state, callback_query)
            msg, keyboard = output_data
        ...
        @self.dp.callback_query_handler(text = ButtonCallbackData.AFFILIATE_PROGRAM_STATISTICS_CALLBACK_DATA, state = self.state)
        async def affiliate_program_statistics_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
            next_state = await self.logic(callback_query)
            output_data = await self.db(next_state, callback_query)
            msg, keyboard = output_data
