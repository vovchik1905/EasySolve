...
CALLBACK_DATA_LIST = [ButtonCallbackData.SUPPORT_ORDER_SETTING_CALLBACK_DATA,
                        ButtonCallbackData.PERCENT_COMPLET_ORDER_SETTING_CALLBACK_DATA,
                        ButtonCallbackData.PREFERRED_BUDGET_ORDER_SETTING_CALLBACK_DATA,
                        ButtonCallbackData.TYPE_ORDER_SETTING_CALLBACK_DATA,
                        ButtonCallbackData.BACK_CALLBACK_DATA]


class AdditionalOrderSettingsMenuStep(StepModel):
    ...
    def get_order_importance(tg_id: int) -> Any:
        created_order_id = AdditionalOrderSettingsMenuStep.get_created_order_id(tg_id)
    ...
    async def logic(self, data: Any) -> State:
        ...
        if callback_data == ButtonCallbackData.BACK_CALLBACK_DATA:
            order_importance = AdditionalOrderSettingsMenuStep.get_order_importance(data.from_user.id)
        ...
        elif callback_data == ButtonCallbackData.SUPPORT_ORDER_SETTING_CALLBACK_DATA:
            return BotStates.GET_ORDER_SUPPORT_DATE_STEP
        elif callback_data == ButtonCallbackData.PERCENT_COMPLET_ORDER_SETTING_CALLBACK_DATA:
            return BotStates.GET_ORDER_PERCENT_COMPLET_STEP
        elif callback_data == ButtonCallbackData.PREFERRED_BUDGET_ORDER_SETTING_CALLBACK_DATA:
            return BotStates.GET_ORDER_PREFERRED_BUDGET_STEP
        elif callback_data == ButtonCallbackData.TYPE_ORDER_SETTING_CALLBACK_DATA:
            return BotStates.GET_ORDER_FORMAT_STEP
        
        return self.state
    
    async def db(self, next_state: State, data: Any) -> Any:
        callback_data = data["data"]
        order_id = AdditionalOrderSettingsMenuStep.get_created_order_id(data.from_user.id)
