...
class GetOrderPreferredBudgetStep(StepModel):
    ...
    def get_order_importance(tg_id: int) -> Any:
        created_order_id = GetOrderPreferredBudgetStep.get_created_order_id(tg_id)
    ...
    async def db(self, next_state: State, data: Any) -> Any:
        callback_data = data.text
        order_id = GetOrderPreferredBudgetStep.get_created_order_id(data.from_user.id)
        if next_state == BotStates.GET_UP_ORDER_SETTINGS_STEP:
            order_preferred_budget = int(callback_data)
            order_request.update_order_preferred_budget(order_id, order_preferred_budget)
