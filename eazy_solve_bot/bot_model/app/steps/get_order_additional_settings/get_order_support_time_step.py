...
class GetOrderSupportTimeStep(StepModel):
    ...
    @staticmethod
    def update_created_order_support_time(tg_id: int, order_deadline_time: tuple) -> Any:
        created_order_id = GetOrderSupportTimeStep.get_created_order_id(tg_id)
        if created_order_id is None:
            return None
        #print(order_deadline_time)
        if len(order_deadline_time) == 2:
            deadline_time = (order_deadline_time[0], order_deadline_time[1], 59)
            order_request.update_order_support_time(created_order_id, deadline_time)
        else:
            order_request.update_order_support_time(created_order_id, order_deadline_time)
        return created_order_id
    ...
    async def logic(self, data: Any) -> State:
        ...
        if callback_data in ButtonCallbackData.GET_ORDER_TIME_CALLBACK_DATA:
            return BotStates.GET_ORDER_SUPPORT_COMMENT_STEP
        return self.state
    
    async def db(self, next_state: State, data: Any) -> Any:
        callback_data = data["data"]
        order_id = GetOrderSupportTimeStep.get_created_order_id(data.from_user.id)
        if callback_data in ButtonCallbackData.GET_ORDER_TIME_CALLBACK_DATA:
            order_deadline_time = (23,59,59)
            GetOrderSupportTimeStep.update_created_order_support_time(data.from_user.id, order_deadline_time)
    ...
    def view(self) -> None:
        ...
        async def setting_up_language_step_view(message: types.Message, state: FSMContext):
            if GetOrderSupportTimeStep.check_order_time(message.text):
                order_deadline_time = tuple(map(int, list(message.text.split(":"))))
                GetOrderSupportTimeStep.update_created_order_support_time(message.from_user.id, order_deadline_time)
                next_state = BotStates.GET_ORDER_SUPPORT_COMMENT_STEP
                order_id = GetOrderSupportTimeStep.get_created_order_id(message.from_user.id)
