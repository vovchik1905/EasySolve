...
#DATE_MONTH = 1
#DATE_DAY = 1
#ORDER_DEADLINE_DATE_CALLBACK_DATA = [f"{ButtonCallbackData.DAY_CALLBACK_DATA}_{DATE_YEAR}_{DATE_MONTH}_{day_num}" for day_num in range(1,32)]
ORDER_DEADLINE_DATE_CALLBACK_DATA = [f"{ButtonCallbackData.DAY_CALLBACK_DATA}_{day_num}" for day_num in range(1,32)]
#ORDER_DEADLINE_DATE_CALLBACK_DATA.append(ButtonCallbackData.LAST_MONTH_CALLBACK_DATA)
#ORDER_DEADLINE_DATE_CALLBACK_DATA.append(ButtonCallbackData.NEXT_MONTH_CALLBACK_DATA)
...
class GetOrderDeadlineDateStep(StepModel):
    ...
    @staticmethod
    def update_created_order_deadline_date(tg_id: int, order_deadline_date: tuple) -> Any:
        created_order_id = GetOrderDeadlineDateStep.get_created_order_id(tg_id)
        if created_order_id is None:
            return None
        order_request.update_order_deadline_date(created_order_id, order_deadline_date)
        return created_order_id
    ...
    def get_year_and_month(order_id: int) -> tuple:
            get_order_deadline_date = order_request.get_order_deadline_date(order_id)
    ...
    async def logic(self, data: Any) -> State:
        ...
        #    order_importance = GetOrderDeadlineDataStep.get_order_importance(data.from_user.id)
        ...
        if callback_data in ORDER_DEADLINE_DATE_CALLBACK_DATA:
            #order_id = GetOrderDeadlineDateStep.get_created_order_id(data.from_user.id)
            #order_year, order_month = GetOrderDeadlineDateStep.get_year_and_month(order_id)
            #order_day = int(list(str(list(callback_data.split(ButtonCallbackData.DAY_CALLBACK_DATA))[1]).split("_"))[1])
            #order_date_obj = datetime.datetime(order_year, order_month, order_day)
            #if order_date_obj >= datetime.datetime(datetime.datetime.today().year, datetime.datetime.today().month, datetime.datetime.today().day):
            return BotStates.GET_ORDER_DEADLINE_TIME_STEP
        return self.state
    ...
    async def db(self, next_state: State, data: Any) -> Any:
        ...
        year, month = GetOrderDeadlineDateStep.get_year_and_month(order_id)
        today_date = 1
        if next_state == self.state:
            if callback_data == ButtonCallbackData.LAST_MONTH_CALLBACK_DATA:
                #year, month = get_year_and_month(order_id)
                if month == 1:
                    year -= 1
                    month = 12
                else:
                ...
                if order_date_obj >= datetime.datetime(datetime.datetime.today().year, datetime.datetime.today().month, 1):
                    #print("!!@!!")
                    GetOrderDeadlineDateStep.update_created_order_deadline_date(data.from_user.id,
                                                                                last_month_order_deadline_date)
                    date_keyboard = GetDateMenuKeyboard(language_name, year, month, day)
                    date_message = Messages(language_name).GET_ORDER_DEADLINE_DATE_MESSAGE
                ...
                if order_date_obj >= datetime.datetime.today():
                    GetOrderDeadlineDateStep.update_created_order_deadline_date(data.from_user.id, next_month_order_deadline_date)
                date_keyboard = GetDateMenuKeyboard(language_name, year, month, day)
                date_message = Messages(language_name).GET_ORDER_DEADLINE_DATE_MESSAGE
                next_month_view = (date_message, date_keyboard.get_date_menu_keyboard)
                return next_month_view
        else:
            #year, month, day = [list(str(list(callback_data.split(
            #                                ButtonCallbackData.DAY_CALLBACK_DATA))[1]).split("_"))[i] for i in range(1,4)]
            #year, month = get_year_and_month(order_id)
            ...
            #print(order_deadline_date)
            order_date_obj = datetime.datetime(year, month, day)
            if order_date_obj >= datetime.datetime(datetime.datetime.today().year,
                                                    datetime.datetime.today().month,
                                                    datetime.datetime.today().day):
                GetOrderDeadlineDateStep.update_created_order_deadline_date(data.from_user.id, order_deadline_date)
    ...
    def view(self) -> None:
        ...
        async def setting_up_language_step_view(message: types.Message, state: FSMContext):
            if GetOrderDeadlineDateStep.check_order_date(message.text):
                order_deadline_date = tuple(map(int, list(message.text.split("."))))
                GetOrderDeadlineDateStep.update_created_order_deadline_date(message.from_user.id, order_deadline_date)
                next_state = BotStates.GET_ORDER_DEADLINE_TIME_STEP
                ...
                msg, keyboard = view_object.GET_ORDER_DEADLINE_TIME_STEP
