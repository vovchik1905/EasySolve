...
class BalanceStep(StepModel):
    ...
    async def logic(self, data: Any) -> State:
        ...
        if callback_data == ButtonCallbackData.CASH_IN_CALLBACK_DATA:
            return BotStates.CASH_IN_STEP

        elif callback_data == ButtonCallbackData.CASH_OUT_CALLBACK_DATA:
            return BotStates.CASH_OUT_STEP
    ...
    def view(self) -> None:
        @self.dp.callback_query_handler(text = ButtonCallbackData.CASH_IN_CALLBACK_DATA, state = self.state)
        async def cash_in_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
        ...
        @self.dp.callback_query_handler(text = ButtonCallbackData.CASH_OUT_CALLBACK_DATA, state = self.state)
        async def cash_out_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
            next_state = await self.logic(callback_query)
            output_data = await self.db(next_state, callback_query)
            msg, keyboard = output_data
