...
class SupportStep(StepModel):
    ...
    async def logic(self, data: Any) -> State:
        ...
        if callback_data == ButtonCallbackData.BECOME_A_PARTNER_CALLBACK_DATA:
            return BotStates.BECOME_A_PARTNER_STEP

        elif callback_data == ButtonCallbackData.BECOME_A_SOLVER_CALLBACK_DATA:
            return BotStates.BECOME_A_SOLVER_STEP
        
        elif callback_data == ButtonCallbackData.ASK_A_QUESTION_CALLBACK_DATA:
            return BotStates.ASK_A_QUESTION_STEP
        
        elif callback_data == ButtonCallbackData.FAQ_CALLBACK_DATA:
            return BotStates.FAQ_STEP
        
        elif callback_data == ButtonCallbackData.BACK_CALLBACK_DATA:
            RuntimeStorageUser.privileges = main_request.get_privileges_with_tg_id(data.from_user.id)
            if RuntimeStorageUser.privileges == 1:
    ...
    def view(self) -> None:
        @self.dp.callback_query_handler(text = ButtonCallbackData.BECOME_A_PARTNER_CALLBACK_DATA, state = self.state)
        async def become_a_partner_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
        ...
        @self.dp.callback_query_handler(text = ButtonCallbackData.BECOME_A_SOLVER_CALLBACK_DATA, state = self.state)
        async def become_a_solver_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
            next_state = await self.logic(callback_query)
            output_data = await self.db(next_state, callback_query)
            msg, keyboard = output_data
        ...
        @self.dp.callback_query_handler(text = ButtonCallbackData.ASK_A_QUESTION_CALLBACK_DATA, state = self.state)
        async def ask_a_question_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
            next_state = await self.logic(callback_query)
            output_data = await self.db(next_state, callback_query)
            msg, keyboard = output_data
        ...
        @self.dp.callback_query_handler(text = ButtonCallbackData.FAQ_CALLBACK_DATA, state = self.state)
        async def faq_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
            next_state = await self.logic(callback_query)
            output_data = await self.db(next_state, callback_query)
            msg, keyboard = output_data
