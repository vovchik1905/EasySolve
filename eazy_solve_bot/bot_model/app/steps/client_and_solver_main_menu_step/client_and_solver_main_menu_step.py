...
class ClientAndSolverMainMenuStep(StepModel):
    ...
    async def logic(self, data: Any) -> State:
        ...
        elif callback_data == ButtonCallbackData.SOLVER_MENU_CALLBACK_DATA:
            return BotStates.SOLVER_MENU_STEP
    ...
    def view(self) -> None:
        ...
        @self.dp.callback_query_handler(text = ButtonCallbackData.SOLVER_MENU_CALLBACK_DATA, state = self.state)
        async def affiliate_program_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
            next_state = await self.logic(callback_query)
            output_data = await self.db(next_state, callback_query)
