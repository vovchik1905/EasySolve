...
class GetOrderCommentStep(StepModel):
    ...
    @staticmethod
    def update_created_order_comment(tg_id: int, order_comment: str) -> Any:
        created_order_id = GetOrderCommentStep.get_created_order_id(tg_id)
        if created_order_id is None:
            return None
        order_request.update_order_comment(created_order_id, order_comment)
        return created_order_id
    ...
    async def logic(self, data: Any) -> State:
        ...
        if GetOrderCommentStep.check_order_comment(order_comment):
            order_importance = GetOrderCommentStep.get_order_importance(data.from_user.id)
    ...
    async def db(self, next_state: State, data: Any) -> Any:
        ...
        if next_state in ORDER_IMPORTANCE_STATES:
            GetOrderCommentStep.update_created_order_comment(data.from_user.id, order_comment)
        order_id = GetOrderCommentStep.get_created_order_id(data.from_user.id)
