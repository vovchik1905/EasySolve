...
class MyProfileStep(StepModel):
    ...
    async def logic(self, data: Any) -> State:
        ...
        if callback_data == ButtonCallbackData.SET_BANKING_DETAILS_CALLBACK_DATA:
            return BotStates.SET_BANKING_DETAILS_STEP

        elif callback_data == ButtonCallbackData.SET_CURRENCY_CALLBACK_DATA:
            return BotStates.SET_CURRENCY_STEP

        elif callback_data == ButtonCallbackData.SET_LANGUAGE_CALLBACK_DATA:
            return BotStates.SET_LANGUAGE_STEP
    ...
    def view(self) -> None:
        @self.dp.callback_query_handler(text = ButtonCallbackData.SET_BANKING_DETAILS_CALLBACK_DATA, state = self.state)
        async def set_banking_details_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
        ...
        @self.dp.callback_query_handler(text = ButtonCallbackData.SET_CURRENCY_CALLBACK_DATA, state = self.state)
        async def set_currency_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
            next_state = await self.logic(callback_query)
            output_data = await self.db(next_state, callback_query)
            msg, keyboard = output_data
        ...
        @self.dp.callback_query_handler(text = ButtonCallbackData.SET_LANGUAGE_CALLBACK_DATA, state = self.state)
        async def set_language_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
            next_state = await self.logic(callback_query)
            output_data = await self.db(next_state, callback_query)
            msg, keyboard = output_data
