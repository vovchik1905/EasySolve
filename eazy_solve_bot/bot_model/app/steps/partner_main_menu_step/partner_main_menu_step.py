...
class PartnerMainMenuStep(StepModel):
    ...
    async def logic(self, data: Any) -> State:
        ...
        elif callback_data == ButtonCallbackData.PARTNER_CONTROL_PANEL_CALLBACK_DATA:
            return BotStates.PARTNER_CONTROL_MENU
        return self.state
    ...
    def view(self) -> None:
        ...
        @self.dp.callback_query_handler(text = ButtonCallbackData.PARTNER_CONTROL_PANEL_CALLBACK_DATA, state = self.state)
        async def partner_control_panel_callback_button(callback_query: types.CallbackQuery, state: FSMContext):
            """"""
            next_state = await self.logic(callback_query)
            output_data = await self.db(next_state, callback_query)
            msg, keyboard = output_data
