...
NEW_IMPORTANT_ORDER_CALLBACK_DATA = [ButtonCallbackData.NEW_NOW_ORDER_IMPORTANCE_CALLBACK_DATA,
                                    ...
                                    ButtonCallbackData.NEW_IMPORTANT_ORDER_DURATION_TIME_CALLBACK_DATA,
                                    ButtonCallbackData.UP_ORDER_SETTINGS_CALLBACK_DATA,
                                    ButtonCallbackData.BACK_CALLBACK_DATA]


class MainImportantOrderStep(StepModel):
    ...
    def update_created_order_status(tg_id: int, order_status_name: str) -> Any:
        created_order_id = MainImportantOrderStep.get_created_order_id(tg_id)
        if created_order_id is None:
            return None
    ...
    async def logic(self, data: Any) -> State:
        ...
        if callback_data == ButtonCallbackData.NEW_NOW_ORDER_IMPORTANCE_CALLBACK_DATA:
        ...
        elif callback_data == ButtonCallbackData.NEW_IMPORTANT_ORDER_DURATION_TIME_CALLBACK_DATA:
            ...
            order_id = MainImportantOrderStep.get_created_order_id(data.from_user.id)
            ...
            MainImportantOrderStep.update_created_order_status(data.from_user.id, STATE_ORDER[OrderStateEnum.ORDER_FROZEN])
            return BotStates.NEW_ORDER_BY_BOT_STEP
        return self.state
    ...
    def view(self) -> None:
        @self.dp.callback_query_handler(lambda call: call.data in NEW_IMPORTANT_ORDER_CALLBACK_DATA, state = self.state)
