...
class MainImportantOrderMenuKeyboard:
    def __init__(self, language: str, order_id: int) -> None:
        ...
        def check_deadline_time_complete(order_id: int) -> Any:
            order_deadline_time = order_request.get_order_deadline_time(order_id)
            if order_deadline_time is None:
                return None
            return order_deadline_time

        def check_duration_time_complete(order_id: int) -> Any:
        ...
        self.new_standart_order_importance_btn = InlineKeyboardButton(self.button_headings.NEW_IMPORTANT_ORDER_IMPORTANCE_HEADING,
        ...
        if check_duration_time_complete(order_id) is None:
            time_duration_btn_heading = f"{self.button_headings.NEW_IMPORTANT_ORDER_DURATION_TIME_HEADING}: {NONE_VALUE}"
        else:
            time_duration_btn_heading = f"{self.button_headings.NEW_IMPORTANT_ORDER_DURATION_TIME_HEADING}: {DONE_VALUE}"
        
        self.new_standart_order_duration_time = InlineKeyboardButton(time_duration_btn_heading,
                                                        callback_data=ButtonCallbackData.NEW_IMPORTANT_ORDER_DURATION_TIME_CALLBACK_DATA)
        ...
        self.main_important_order_menu_keyboard = InlineKeyboardMarkup(row_width=2)
        self.main_important_order_menu_keyboard.add(self.new_standart_order_importance_btn,
                                                ...
                                                self.new_standart_order_duration_time)
                                                
        self.main_important_order_menu_keyboard.add(self.up_order_settings_btn)
        self.main_important_order_menu_keyboard.add(self.send_order_to_solvers_btn)
        self.main_important_order_menu_keyboard.add(self.back_btn)