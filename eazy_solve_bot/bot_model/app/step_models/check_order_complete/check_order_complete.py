...
class CheckOrderComplete:
    ...
    def get_order_number_of_tasks(self) -> Any:
        order_number_of_tasks = order_request.get_order_number_of_tasks(self.order_id)
        if order_number_of_tasks is None:
            return None
        return order_number_of_tasks
    
    def get_order_duration_time(self) -> Any:
        order_duration_time = order_request.get_order_duration_time(self.order_id)
        if order_duration_time is None:
            return None
        return order_duration_time

    def check_all_standart_order_settings(self) -> bool:
    ...
    def check_all_now_order_settings(self) -> bool:
        subject_complete_value = self.get_order_subject_complete()
        topic_complete_value = self.get_order_topic_complete()
        comment_complete_value = self.get_order_comment_complete()
        files_complete_value = self.get_order_files_complete()
        duration_time_complete_value = self.get_order_duration_time()
        number_of_tasks_complete_value = self.get_order_number_of_tasks()

        all_order_values = (subject_complete_value,
                            topic_complete_value,
                            comment_complete_value,
                            files_complete_value,
                            duration_time_complete_value,
                            number_of_tasks_complete_value)
